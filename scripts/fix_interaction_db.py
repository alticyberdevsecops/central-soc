    ...
    async with engine.begin() as conn:
        ...
        for tenant in tenants:
            ...
            for inc in incidents:
                ...
                # Interaction Fixes: Replace <div> based buttons/tabs with <a> tags
                # Navigation buttons
                verdict_html = re.sub(r'<div class="nav-btn">([^<]+)</div>', r'<a href="#" class="nav-btn">\\1</a>', verdict_html)
                # IOC Tabs
                verdict_html = re.sub(r'<div class="tab">([^<]+)</div>', r'<a href="#" class="tab">\\1</a>', verdict_html)
                verdict_html = re.sub(r'<div class="tab active">([^<]+)</div>', r'<a href="#" class="tab active">\\1</a>', verdict_html)
                
                # Ensure a.nav-btn and a.tab have cursor: pointer and no text decoration
                if '.nav-btn {' in verdict_html and 'cursor: pointer' not in verdict_html:
                    verdict_html = verdict_html.replace('.nav-btn {', '.nav-btn {\\n            cursor: pointer;\\n            text-decoration: none;')
                if '.tab {' in verdict_html and 'cursor: pointer' not in verdict_html:
                    verdict_html = verdict_html.replace('.tab {', '.tab {\\n            cursor: pointer;\\n            text-decoration: none;')

                await conn.execute(text("UPDATE incidents SET ai_verdict = :v WHERE id = :id"), {"v": verdict_html, "id": inc.id})
                print(f"Patched report themes and interactions for incident {inc.id} in schema {schema}")
