...
# Using direct bcrypt to avoid passlib internal bug checks
def get_password_hash(password):
    return bcrypt.hashpw(password.encode('utf-8'), bcrypt.gensalt()).decode('utf-8')

DB_URL = os.environ.get(
    "DATABASE_URL_SEED",
    "postgresql://soc_admin:soc_secret_2024@localhost:5432/central_soc"
)
...
def seed():
    ...
    cur.execute("""
        ...
        RETURNING id
    """, (super_admin_id, get_password_hash(SUPER_ADMIN_PASSWORD)))
    result = cur.fetchone()
    if result:
    ...
    cur.execute("""
        INSERT INTO users (id, tenant_id, email, full_name, hashed_password, role)
        VALUES (%s, NULL, 'admin@admin.com', 'Simple Admin', %s, 'super_admin')
        ON CONFLICT (email) DO UPDATE SET hashed_password = EXCLUDED.hashed_password
    """, (str(uuid.uuid4()), get_password_hash("admin")))
    conn.commit()
    print("✅ Simple Admin: admin@admin.com / admin")
    ...
    for tenant_def in TENANTS:
        ...
        cur.execute("""
            ...
            RETURNING id
        """, (tenant_id, tenant_def["name"], tenant_def["slug"], tenant_def["description"], tenant_def["contact_email"]))
        result = cur.fetchone()
        if result:
        ...
        """, (tenant_id, admin_email, f"Admin - {tenant_def['name']}", get_password_hash(SUPER_ADMIN_PASSWORD)))
        ...
        cur.execute("""
            ...
            ON CONFLICT (email) DO NOTHING
        """, (tenant_id, analyst_email, f"SOC Analyst - {tenant_def['name']}", get_password_hash(ANALYST_PASSWORD)))

        # Connector config (mock — no real API keys)
        vendor = tenant_def["primary_vendor"]
        cur.execute("""
            INSERT INTO connector_configs (tenant_id, vendor, label, is_enabled, poll_interval_sec, credentials, last_poll_status)
            VALUES (%s, %s, %s, %s, %s, %s::jsonb, 'ok')
            ON CONFLICT (tenant_id, vendor, label) DO NOTHING
        """, (tenant_id, vendor, f"{tenant_def['name']} Primary", True, 300, json.dumps({"mock": True})))
        ...
        for title, severity, hostname, tactic, technique in incidents:
            ...
            cur.execute("""
                ...
                ON CONFLICT (tenant_id, source_vendor, vendor_incident_id) DO NOTHING
            """, (
